...
const Chart = ({ aspect, title, getTab, childToParent }) => {
  ...
  useEffect(() => {
    const API_URL = `http://demo.thingsboard.io/api/plugins/telemetry/DEVICE/${process.env.REACT_APP_ENITYID}/values/timeseries?keys=temperature,humidity&startTs=${timeStart.getTime().toString()}&endTs=${timeEnd.getTime().toString()}&interval=60000&limit=100`
